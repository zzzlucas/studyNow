  ...
  <a-drawer :title="title" width="1180" destroyOnClose :visible="visible" @close="handleCancel">
    <a-card :bordered="false">
      <a-form :form="form">
        ...
        <a-form-item label="内容内容">
          <j-editor v-model="editor.content"></j-editor>
        </a-form-item>
        <a-form-item label="二维码">
          <j-upload v-decorator="['imageList']" />
        </a-form-item>
        <!-- <a-alert message="二维码内容默认不在本网站前台底部显示" style="margin-top:20px" type="warning" closeText="关闭提示" /> -->
      </a-form>
  ...
  data() {
    return {
      ...
      url: {
        ...
        add4: '/platform/platformBody.controller/contactUs',
    ...
    detail(row) {
      ...
      getAction(this.url.content, { title: this.params, contentTitle: row }).then(res => {
        if (res.code === 200) {
          ...
          this.visible = true
          this.$nextTick(() => {
            this.form.setFieldsValue(pick(this.model, 'contentTitle', 'imageList'))
            this.editor = {
