<template>
  <div class="main">
    ...
    <!-- Department(park) select -->
    <depart-select ref="departSelect" title="登录园区选择" @change="changeDepResult"  />
  </div>
</template>
...
<script>
  ...
  import DepartSelect from '@comp/tools/DepartSelect'
  import { postAction } from '@/api/manage'
  import * as Depart from '@/api/depart'
      ...
      departConfirm(res){
        if(res.success){
          ...
          if(multi_depart==0){
            ...
            const $depart = this.$refs.departSelect
            ...
            Depart.loadList()
              .then(resp => {
                ...
                return Depart.selectDepart(resp.list[0].orgCode)
              })
              .then(resp => {
                this.loginSuccess()
              })
